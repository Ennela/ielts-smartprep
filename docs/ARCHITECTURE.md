...
| GET | `/api/v1/listening/{testId}/result` | JWT | `@PathVariable testId` + userId từ principal (`findByTestIdAndUserUserId`, sai chủ → 404; `ListeningQueryServiceOwnershipTest`) | `ListeningTestResponse` | `controller/ListeningController.java:88-92` |
...
| GET | `/api/v1/history` | JWT | `skill` (READING/LISTENING/WRITING/MOCK_TEST), `from` (ISO date-time), `q`, `page`, `size` (cap 100) | `Page<HistoryFeedItemResponse>` — feed gộp 4 skill (UNION ALL 4 bảng nguồn qua JdbcTemplate, `service/HistoryFeedService.java`), mới nhất trước; `refId` là id trong bảng của skill đó, `title` là nhãn thô của skill, `status` chỉ có với mock test; `historyId` + `timeSpentSeconds` lấy từ `score_history` khớp ±5 s (LATERAL, như các trang per-skill), mock test lấy thời gian = `submitted_at − session.started_at`. Trang `/history` dùng endpoint này thay cho 4 endpoint per-skill | `controller/HistoryFeedController.java` |
