...
export default function DashboardPage() {
  ...
  const radarChartData = weaknessData && weaknessData.accuracies ? Object.keys(weaknessData.accuracies).map(type => ({
    ...
    <div className={styles['dashboard-content']}>
      ...
      <section className={`${styles['dash-hero']} reveal`}>
        <div className={styles['dash-welcome-card']}>
          <div>
          ...
          <div className={styles['dash-welcome-actions']}>
            <button
        ...
        {/* Target Band Card */}
        <div className={styles['dash-target-card']}>
          <p className={styles['dash-target-label']}>Overall Target Band</p>
          <div className={styles['dash-target-band']}>{targetBand}</div>
          <div className={styles['dash-target-chip']}>
      ...
      <section className="reveal reveal-delay-1">
        ...
        <div className={styles['skill-progress-row']}>
          {overview?.skills ? (
            ...
            ['READING', 'WRITING', 'LISTENING'].map(s => (
              <div key={s} className={styles['skill-progress-card']} style={{ cursor: 'pointer' }} onClick={() => navigate(`/${s.toLowerCase()}`)}>
                <div className={styles['spc-header']}>
                  ...
                  <span className={`${styles['spc-band-score']} ${styles[`spc-band-${s.toLowerCase()}`]}`}>—</span>
                </div>
                <p className={styles['spc-skill-name']}>{s.charAt(0) + s.slice(1).toLowerCase()}</p>
                <p className={styles['spc-count']}>Start practicing to track your progress</p>
                <div className={styles['spc-progress-bar']}>
                  <div className={`${styles['spc-progress-fill']} ${styles[`spc-fill-${s.toLowerCase()}`]}`} style={{ width: '0%' }} />
                </div>
              </div>
            ))
          )}
      ...
      <section className={`${styles['dash-charts-grid']} reveal reveal-delay-2`}>
        
        {/* Score Trend Line Chart */}
        <div className={styles['trend-section']} style={{ marginBottom: 0 }}>
          <div className={styles['trend-header']}>
            <h2>Score Trends</h2>
            <div className={styles['trend-controls']}>
              <div className={styles['trend-tabs']}>
                {['READING', 'WRITING', 'LISTENING'].map(s => (
                  <button
                    key={s}
                    className={`${styles['trend-tab']} ${trendSkill === s ? styles.active : ''}`}
              ...
              <select
                className={styles['trend-period-select']}
                value={trendPeriod}
          ...
          <div className={styles['trend-chart-wrapper']}>
      ...
      <section className={`${styles['dash-charts-grid']} reveal reveal-delay-2`}>
        ...
        <div className={styles['trend-section']} style={{ marginBottom: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div>
            <div className={styles['trend-header']}>
            ...
            <div style={{ maxHeight: '200px', overflowY: 'auto', marginBottom: '20px' }}>
              {weaknessData?.accuracies && Object.keys(weaknessData.accuracies).length > 0 ? (
                <div className={styles['history-table']} style={{ border: '1px solid var(--color-border)' }}>
                  <div className={styles['ht-header']} style={{ gridTemplateColumns: '1fr 1fr' }}>
                  ...
                  {Object.keys(weaknessData.accuracies).map((type, i) => {
                    ...
                    return (
                      <div key={i} className={styles['ht-row']} style={{ gridTemplateColumns: '1fr 1fr' }}>
                        <span>{type}</span>
                ...
                <div className={styles['trend-empty']} style={{ minHeight: '120px' }}>
      ...
      <div className={`${styles['history-section']} reveal reveal-delay-3`}>
        ...
        {history?.items?.length > 0 ? (
          <>
            <div className={styles['history-table']}>
              <div className={styles['ht-header']}>
                <span>Skill</span>
                <span>Score</span>
                <span>Date</span>
              </div>
              {history.items.map((item, i) => (
                <div key={i} className={styles['ht-row']}>
                  <span className={`${styles['ht-skill']} ${styles[`badge-${item.skillType?.toLowerCase()}`]}`}>
                    {item.skillType}
                  </span>
                  <span className={styles['ht-score']}>{item.score?.toFixed ? item.score.toFixed(1) : item.score}</span>
                  <span className={styles['ht-date']}>
          ...
          <div className={styles['trend-empty']}><p>No history available.</p></div>
        )}
      </div>
    </div>
  );
